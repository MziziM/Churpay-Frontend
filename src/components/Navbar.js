...
import { FaUserPlus, FaSignInAlt, FaSignOutAlt, FaHandHoldingUsd } from 'react-icons/fa';
  ...
  return (
    <nav className="bg-gradient-to-r from-purple-800 to-indigo-700 px-4 py-3 flex items-center justify-between shadow-xl relative z-50 border-b-2 border-yellow-300/30">
      ...
      <div className={`flex-col sm:flex-row sm:flex items-center gap-3 md:gap-6 absolute sm:static bg-gradient-to-r from-purple-900 to-indigo-800 sm:bg-transparent left-0 w-full sm:w-auto px-4 py-6 sm:p-0 top-full sm:top-auto transition-all duration-300 shadow-xl sm:shadow-none z-40 ${menuOpen ? "flex" : "hidden sm:flex"}`}>
        ...
        {!loggedIn && <Link to="/login" className="bg-gradient-to-r from-yellow-400 to-yellow-300 hover:from-yellow-500 hover:to-yellow-400 text-purple-900 font-bold px-4 py-2 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center gap-1.5">
          <FaSignInAlt className="text-sm" /> Login
        </Link>}
        {!loggedIn && <Link to="/signup" className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-yellow-300 font-bold px-4 py-2 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center gap-1.5">
          <FaUserPlus className="text-sm" /> Sign Up
        </Link>}
        {loggedIn && (
          <button
            className="bg-gradient-to-r from-red-500 to-pink-600 hover:from-red-600 hover:to-pink-700 text-white font-bold px-4 py-2 rounded-xl transition-all shadow-md hover:shadow-lg mt-2 sm:mt-0 flex items-center gap-1.5"
            ...
            <div className="fixed inset-0 z-30 flex items-center justify-center bg-black bg-opacity-70" onClick={() => setShowGiveOptions(false)}>
              ...
              >
                ...
                <div className="flex flex-col gap-2">
                  <button
                    type="submit"
                    className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-yellow-300 font-bold py-3 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl text-lg tracking-wide transform hover:translate-y-[-1px] active:translate-y-[1px]"
                  >
                    Give Now
                  </button>
                  <button
