...
export default function Projects() {
	...
	React.useEffect(() => {
		...
		setLoading(true);
		axios
			.get(`https://churpay-backend.onrender.com/api/projects/${id}`)
			.then((res) => {
				setProject(res.data);
				setLoading(false);
			})
	...
	const handleWhatsApp = () =>
		window.open(
			`https://wa.me/?text=Support this project on ChurPay: ${shareUrl}`,
			"_blank"
		);
	...
	if (loading) {
		return (
			<div className="flex items-center justify-center min-h-screen text-xl text-purple-700">
				Loading...
			</div>
		);
	}
	...
	return (
		<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-700 to-indigo-500 dark:from-purple-900 dark:to-gray-900 py-8 px-2">
			<div className="w-full max-w-5xl bg-white dark:bg-gray-900 rounded-3xl shadow-2xl px-10 py-14 flex flex-col items-center">
				<h1 className="text-4xl md:text-5xl font-extrabold text-yellow-300 mb-6 text-center drop-shadow-lg">
					Projects
				</h1>
				{/* Project Image */}
				<img
				...
				<div className="flex gap-4 mb-8 justify-center">
					...
					>
						...
						>
							...
							<path
								fill="#fff"
								d="M20.61 17l.45-2.94h-2.82v-1.9c0-.8.39-1.58 1.64-1.58h1.27V8.01S20.06 8 19.21 8c-2.36 0-3.49 1.42-3.49 4.03v2.03H13v2.94h2.72v7.12c.55.09 1.12.14 1.7.14s1.15-.05 1.7-.14V17h2.49z"
							/>
						</svg>
						Facebook
					</button>
					...
					>
						...
						>
							<path
								fill="currentColor"
								d="M10.59 13.41a1 1 0 0 1 0-1.41l3-3a1 1 0 1 1 1.41 1.41l-3 3a1 1 0 0 1-1.41 0Z"
							/>
							<path
								fill="currentColor"
								d="M13 10.59a1 1 0 0 1 1.41 0l3 3a1 1 0 1 1-1.41 1.41l-3-3a1 1 0 0 1 0-1.41Z"
							/>
							<path
								fill="currentColor"
								d="M5 13a7 7 0 1 1 14 0 7 7 0 0 1-14 0Z"
							/>
						</svg>
						Copy Link
					</button>
				...
				{showThankYou && (
					<div className="mt-8 bg-green-50 border-l-4 border-green-400 p-6 rounded-xl shadow max-w-md w-full flex flex-col items-center">
						...
						<div className="mb-2 text-gray-800">
							Your giving to <b>{project.title}</b> was received.
						</div>
						{lastAmount && (
